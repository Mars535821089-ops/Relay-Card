...
def extract_last_goal(transcript: str) -> str:
    # 实现: 用你的工具特有方式抽 "current task"
    return "(待实现)"


def extract_done_signals(transcript: str) -> list[str]:
    # 实现: 从 transcript 抽 "completed" 信号
    return []
...
**准备好写你的 adapter 了吗？** 提 PR 之前先在 [GitHub Discussions](https://github.com/Mars535821089-ops/Relay-Card/discussions) 聊聊想法，避免重造轮子。